// sahou（卅）VS Code 扩展主代码（内置版）。
// 1) sahou.run：在终端运行当前 .saho 文件；
// 2) 内置极简 LSP 客户端：启动 `sahou lsp`，把语法诊断映射为编辑器波浪线；
// 3) 文档格式化：调用 `sahou 格式`；
// 4) 任务提供者：Ctrl+Shift+B 直接运行当前文件。
// 无 npm 依赖，直接使用 vscode API 与 child_process。
// 内置版增强：自动探测随产品分发的解释器（extensions/sahou/bin/sahou.exe），
// 用户无需安装 sahou 或配置任何路径即可获得全部语言功能。
...
// 随产品内置的解释器（打包后位于 resources/app/extensions/sahou/bin/sahou.exe）
function builtinExePath() {
  try {
    const exe = path.join(__dirname, "bin", "sahou.exe");
...
// 解析顺序：用户显式配置的路径 > 产品内置解释器 > PATH 中的 sahou
function sahouPath() {
  const configured = vscode.workspace.getConfiguration("sahou").get("path", "").trim();
  if (configured) return configured;
  const builtin = builtinExePath();
...
class SahouRunTaskProvider {
  provideTasks() {
    const editor = vscode.window.activeTextEditor;
    if (!editor || editor.document.languageId !== "sahou") return [];
    const exe = sahouPath();
    const file = editor.document.fileName;
    const isBuiltin = !!builtinExePath() && exe === builtinExePath();
    const execution = new vscode.ProcessExecution(exe, ["run", file], {
      cwd: path.dirname(file),
    });
    const task = new vscode.Task(
      { type: "sahou", task: "运行当前文件" },
...
// ---------- 文档格式化：调用 `sahou 格式`（就地格式化，借临时文件取回结果） ----------

function formatWithSahou(doc) {
  return new Promise((resolve, reject) => {
    const exe = sahouPath();
    const dir = os.tmpdir();
    const tmp = path.join(dir, "sahou-format-" + Date.now() + "-" + process.pid + ".saho");
    fs.writeFile(tmp, doc.getText(), "utf8", (werr) => {
      if (werr) return reject(werr);
      execFile(exe, ["格式", tmp], { timeout: 15000 }, (cerr) => {
        if (cerr) {
          fs.unlink(tmp, () => {});
          return reject(cerr);
        }
        fs.readFile(tmp, "utf8", (rerr, formatted) => {
          fs.unlink(tmp, () => {});
...
function startLsp(context) {
  ...
  const exe = sahouPath();
  try {
    lspProcess = spawn(exe, ["lsp"], { shell: false, windowsHide: true });
  } catch (err) {
...
function activate(context) {
  ...
  // 任务：运行当前文件（Ctrl+Shift+B / 任务列表）
  context.subscriptions.push(
    vscode.tasks.registerTaskProvider("sahou", new SahouRunTaskProvider())
  );
...
function registerLanguageFeatures(context) {
  ...
  // 文档格式化（Shift+Alt+F / 右键“格式化文档”）
  const selector = { language: "sahou" };
  context.subscriptions.push(
    vscode.languages.registerDocumentFormattingEditProvider(selector, {
      provideDocumentFormattingEdits(doc) {
        return formatWithSahou(doc);
      },
    })
  );
}
